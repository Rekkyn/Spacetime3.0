...
public class RenderSFHelper {
    ...
    public static void renderVortex(World world, float partialTicks) {
        ...
        float size = 3F;
        tessellator.addVertexWithUV(-size, size, 0.0D, 0, 1);
        tessellator.addVertexWithUV(size, size, 0.0D, 1, 1);
        tessellator.addVertexWithUV(size, -size, 0.0D, 1, 0);
        tessellator.addVertexWithUV(-size, -size, 0.0D, 0, 0);
        tessellator.draw();
    ...
    public static void renderBeams(World world, float partialTicks) {
        ...
        if (time > 0) {
            ...
            for (int i = 0; (float) i < 15; ++i) {
                ...
                tessellator.setColorRGBA_I(13158655, 64);//(int) (127 * a));
